...
@login_required(login_url=SAM_login_url)
def paper_review_pcc(request,paperId):
    paper=Paper.objects.get(id=paperId)
    token={}
    token['paper']=paper
    return render_to_response('common/paper_review_pcc.html',token)

@login_required(login_url=SAM_login_url)
def review_rate_pcc(request,paperId):
    paper=Paper.objects.get(id=paperId)
    user = request.user
    if request.method == 'POST':
        ...
        if form.is_valid():
            new = ReviewRating(reviwer_id=user.id, paper_id=paper.id, review=form.cleaned_data["review"],
                               rating=form.cleaned_data["rating"],is_Final=True)
            new.save()
            return HttpResponseRedirect(resolve_url('view_papers_to_review'))
    ...
    token['form'] = form
    return render_to_response('common/review_rate_pcc.html', token)
...
def assigned_reviewer(request, paperId,reviewerId):
    ...
    if paper.numofreviewers < 3:
        ...
        new_add.save()
        notification = Notification()
        users = PCM.objects.get(id=reviewerId)
        recipient = [users]
        notification.save()
...
def view_requests_pcc(request):
    token={}
    paper_requests = papers_selection.objects.filter(decisions=False)
    for pap in paper_requests:
        paper=Paper.objects.get(id=pap.selected_paper_id)
        if paper.numofreviewers >= 3:
            pap.delete()
    paper_selected=papers_selection.objects.filter(decisions=False)
    token['paper_selected']=paper_selected
...
def paper_approve(request,Id):
    ...
    if num_of_reviewers >= 3:
        pap.delete()
        return HttpResponseRedirect(resolve_url('view_requests_pcc'))
...
def reviewRating(request,paperId):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            notification.save()
            notification.sendNotification('REVIEW_RESULTS', recipient)
